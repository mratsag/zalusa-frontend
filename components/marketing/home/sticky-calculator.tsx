...
// homepage-v2.php STICKY CALCULATOR (floating widget) + includes/kargo-hesapla-v2.php portu.
// - Sağ-alt sabit widget; scrollY > 600'de görünür (toggleStickyCalc / scroll listener).
// - Pill (kapalı) ↔ Panel (açık). Panel içinde "Kargo Hesapla" formu.
// - Ülkeler GET {NEXT_PUBLIC_API_URL}/api/countries (public). Bayraklar flagcdn.com.
// - "Hesapla" → girdileri toplayıp /yurtdisi-kargo-fiyat-hesaplama'ya redirect (hesaplama o sayfada).
//   ⚠️ O pricing sayfası henüz portlanmadı → şimdilik 404'e gider.

const API = process.env.NEXT_PUBLIC_API_URL;

type Opt = { code: string; name: string; flag: string }; // flag: emoji veya url
...
export function StickyCalculator() {
  ...
  // Scroll görünürlüğü (PHP: scrollY < 600 → hidden)
  useEffect(() => {
  ...
  // Ülkeleri API'den yükle (flagcdn + Türkçe adlar)
  useEffect(() => {
    if (!API) return;
  ...
  return (
    <div
      id="sticky-calculator"
      className={`fixed bottom-5 left-5 md:bottom-6 md:left-6 z-40 ${visible ? "" : "hidden"}`}
    >
      {/* Pill (kapalı) */}
      {!expanded && (
        <button
      ...
      {expanded && (
        <div className="w-[calc(100vw-2.5rem)] sm:w-[400px] max-h-[78vh] overflow-y-auto bg-white rounded-2xl shadow-[0_24px_60px_-12px_rgba(15,23,42,0.25),0_8px_20px_-6px_rgba(15,23,42,0.12)] border border-slate-200 ring-1 ring-slate-900/[0.02]">
          ...
          <div className="p-4">
            <div id="calculator-form" className="bg-white/95 backdrop-blur p-6 md:p-7 rounded-2xl border border-slate-200/80 ring-1 ring-slate-900/[0.03] shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_-12px_rgba(15,23,42,0.12)] relative scroll-mt-24">
              ...
              {error && (
                <div className="mb-4 p-3 rounded-xl text-sm font-medium text-center bg-red-50 text-red-700 border border-red-200">
                  {error}
                </div>
              )}
              ...
              <form className="space-y-6" noValidate onSubmit={submit}>
                          ...
                          <ul role="listbox" className="max-h-48 overflow-y-auto py-1">
                            {filtered.map((c) => (
                              <li
                                ...
                                className={`flex items-center gap-3 py-2 px-4 cursor-pointer hover:bg-blue-50 text-slate-900 font-medium text-sm ${
                                  c.code === selected.code ? "bg-blue-100/80" : ""
                                }`}
                              >
                                <Flag flag={c.flag} />
                                {c.name}
                    ...
                    {TYPES.map((t) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center justify-center space-x-2 py-2.5 rounded-xl text-sm transition-all ${
                            active
                              ? "border-2 border-[#7C7CE3]/30 bg-[#7C7CE3]/10 text-[#0000BE] font-bold shadow-sm"
                              : "border border-gray-200 bg-white text-slate-600 font-medium hover:border-gray-300 hover:bg-gray-50"
                          }`}
                        >
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <button
                        type="button"
                        onClick={addPackage}
                        className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-semibold text-[#0000BE] bg-blue-50 border border-blue-200 rounded-lg hover:bg-blue-100 cursor-pointer transition"
                      >
                          ...
                          <div className="flex items-center justify-between mb-2">
                            ...
                            {packages.length > 1 && (
                              <button
                                ...
                                className="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg cursor-pointer transition"
                                title="Paketi sil"
                              >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  type="submit"
                  className="w-full py-3.5 bg-gradient-to-br from-[#4D4DF2] to-[#0000BE] hover:from-[#5959FF] hover:to-[#00009c] hover:-translate-y-0.5 text-white font-semibold rounded-xl transition-all shadow-sm hover:shadow flex items-center justify-center space-x-2"
                >
                  <span>Hesapla</span>
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
