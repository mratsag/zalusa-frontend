...
export async function TasimaModlari() {
  const t = await getTranslations("transportModes");
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-10 md:mb-14">
          ...
          <h2 className="mt-6 text-[36px] sm:text-5xl md:text-[60px] font-semibold tracking-tight leading-[1.05] text-slate-900">
            <span className="text-[#0891B2]">{t("air")}</span>
            <span className="text-slate-300">,</span> <span className="text-[#7C3AED]">{t("land")}</span>
            <span className="text-slate-300">,</span> <span className="text-[#059669]">{t("sea")}</span>
            <span className="block mt-1 md:inline md:mt-0 text-slate-800">{t("titleTail")}</span>
          </h2>
            ...
            >
              ...
              <header className="relative flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className={`mt-1 text-[10.5px] font-bold uppercase tracking-[0.14em] ${m.subtitleColor}`}>{t(`modes.${m.key}.subtitle`)}</p>
                </div>
              ...
              <p className="relative text-[13.5px] text-slate-500 leading-relaxed min-h-[60px]">{t(`modes.${m.key}.desc`)}</p>
              ...
              <div className="relative flex flex-wrap items-center gap-1.5 pt-1">
                {(t.raw(`modes.${m.key}.tags`) as string[]).map((tag) => (
                  <span key={tag} className="text-[10.5px] font-medium text-slate-600 bg-slate-50 border border-slate-200/80 rounded-full px-2.5 py-1">
                    {tag}
                  </span>
                ))}
              </div>
