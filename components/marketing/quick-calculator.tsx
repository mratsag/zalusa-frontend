...
// Varsayılan ülkenin adı aktif dile göre çözülür (önceden sabit "İspanya" idi → /en'de de Türkçe görünüyordu).
function defaultSelected(locale: string): Opt {
  let name = "İspanya";
  try {
    name = new Intl.DisplayNames([locale], { type: "region" }).of(DEFAULT_ISO) || name;
  } catch {
    /* Intl yoksa varsayılan */
  }
  return { code: DEFAULT_ISO, name, flag: "🇪🇸" };
}
const emptyPkg = (): Pkg => ({ weight: "", quantity: "1", width: "", length: "", height: "" });

// DİKKAT: `value` kanonik (Türkçe) kalır — hesaplama sayfasına/backend'e bu değer
// gönderilir (shipmentType). Yalnızca ekranda gösterilen etiket çevrilir.
const TYPES: { value: "Koli" | "Paket" | "Belge"; icon: string }[] = [
  { value: "Koli", icon: "M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" },
  { value: "Paket", icon: "M5 8h14M5 8a2 2 0 110-4h14a2 2 0 110 4M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8m-9 4h4" },
  { value: "Belge", icon: "M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" },
];
...
export function QuickCalculatorForm({ preselectIso2, className = "" }: { preselectIso2?: string; className?: string }) {
  ...
  const [selected, setSelected] = useState<Opt>(() => defaultSelected(locale));
  ...
  useEffect(() => {
    if (!API) return;
    // Ülke adları aktif dile göre (önceden sabit "tr" idi → /en'de de Türkçe görünüyordu).
    let trNames: Intl.DisplayNames | null = null;
    try {
      trNames = new Intl.DisplayNames([locale], { type: "region" });
    } catch {
      trNames = null;
    }
    fetch(`${API}/api/countries`)
      ...
      .then((data: { isoCode: string; countryName: string }[]) => {
        ...
        setCountries(list);
        // Seçili ülkenin adı/bayrağını yeni (dile uygun) listeden tazele — seçim değişmez.
        setSelected((cur) => list.find((c) => c.code === cur.code) ?? cur);
      })
      .catch(() => {
        /* fallback */
      });
  ...
  // Ekranda gösterilen tür adı (kanonik değer değil)
  const typeLabel = t(`types.${pkgType || "Paket"}`);
  ...
  return (
    <div id="calculator-form" className={`bg-white/95 backdrop-blur p-6 md:p-7 rounded-2xl border border-slate-200/80 ring-1 ring-slate-900/[0.03] shadow-[0_1px_2px_rgba(15,23,42,0.04),0_8px_24px_-12px_rgba(15,23,42,0.12)] relative scroll-mt-24 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-[17px] font-semibold text-slate-900 tracking-tight">{t("title")}</h3>
      ...
      <form className="space-y-6" noValidate onSubmit={submit}>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div ref={pickerRef} className="relative">
              ...
              >
                <span className="flex items-center gap-2">
                  ...
                  <span>{selected.name || t("selectCountry")}</span>
                </span>
                  ...
                  <div className="p-2 bg-slate-50 border-b border-slate-200">
                    <div className="flex items-center justify-between gap-2 py-1.5 px-3 rounded-lg bg-white border border-slate-200 mb-2">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelected({ code: "", name: t("selectCountry"), flag: "" });
                          setTouched(true);
                          setPickerOpen(false);
                        }}
          ...
          <div>
            ...
            <input type="text" value={postal} onChange={(e) => setPostal(e.target.value)} placeholder={t("postalPlaceholder")} className={INPUT} />
          </div>
        </div>

        <div>
          <label className="block text-sm font-bold text-slate-900 mb-2">{t("packageType")}</label>
          <div className="grid grid-cols-3 gap-2">
            {TYPES.map((type) => {
              const active = pkgType === type.value;
              return (
                <button
                  key={type.value}
                  type="button"
                  onClick={() => setPkgType(type.value)}
          ...
          {isBelge && (
            <div className="mt-3 p-3 bg-blue-50 border border-blue-200 rounded-xl flex items-center gap-2">
              ...
              <span className="text-sm font-semibold text-[#0000BE]">{t("documentNote")}</span>
            </div>
          )}
        </div>
        ...
        {!isBelge && (
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="block text-sm font-bold text-slate-900">{t("dimensions")}</label>
              ...
              {packages.map((pkg, i) => (
                <div key={i} className="mb-3 p-3 bg-gray-50 border border-gray-200 rounded-xl">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    {packages.length > 1 && (
                      <button type="button" onClick={() => removePackage(i)} className="p-1 text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg cursor-pointer transition" title={t("removePackage")}>
                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <span className="text-xs text-slate-500 mb-1 block">{t("weight")}</span>
                      <div className="relative">
                        ...
                        <span className={UNIT}>{t("unitKg")}</span>
                      </div>
                    </div>
                    <div>
                      <span className="text-xs text-slate-500 mb-1 block">{t("quantity")}</span>
                      <div className="relative">
                        ...
                        <span className={UNIT}>{t("unitPcs")}</span>
                      </div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-3 mt-3">
                    <div>
                      <span className="text-xs text-slate-500 mb-1 block">{t("width")}</span>
                      <div className="relative">
                        <input type="number" min="0" placeholder="0" value={pkg.width} onChange={(e) => updatePackage(i, "width", e.target.value)} className={NUMINPUT} />
                        <span className={UNIT}>{t("unitCm")}</span>
                      </div>
                    </div>
                    <div>
                      <span className="text-xs text-slate-500 mb-1 block">{t("length")}</span>
                      <div className="relative">
                    ...
                    <div>
                      <span className="text-xs text-slate-500 mb-1 block">{t("height")}</span>
                      <div className="relative">
        ...
        <ul className="flex flex-wrap items-center justify-center gap-x-4 gap-y-1 text-xs text-slate-500 mt-3">
          {[t("trust.instant"), t("trust.noCard"), t("trust.free")].map((item) => (
